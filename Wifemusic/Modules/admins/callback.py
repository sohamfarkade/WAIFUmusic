...
from Wifemusic.utils.formatters import seconds_to_min
from Wifemusic.utils.inline import close_markup, stream_markup, stream_markup_timer
from Wifemusic.utils.stream.autoclear import auto_clean
from Wifemusic.utils.thumbnails import get_thumb
from config import (
    BANNED_USERS,
    SUPPORT_CHAT,
    SOUNCLOUD_IMG_URL,
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Wife.pause_stream(chat_id)
        ...
        await Wife.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        if command == "Skip":
            ...
            try:
                ...
                if not check:
                    ...
                    try:
                        return await Wife.stop_stream(chat_id)
                    except:
                        return
            except:
                try:
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await Wife.skip_stream(chat_id, link, video=status, image=image)
            except:
            ...
            try:
                ...
                image = None
            try:
                await Wife.skip_stream(chat_id, file_path, video=status, image=image)
            except:
                return await mystic.edit_text(_["call_6"])
            button = stream_markup(_, chat_id)
            img = await get_thumb(videoid)
            ...
            try:
                await Wife.skip_stream(chat_id, videoid, video=status)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            button = stream_markup(_, chat_id)
            run = await CallbackQuery.message.reply_photo(
            ...
            if videoid == "telegram":
                ...
                try:
                    ...
                    image = None
            try:
                await Wife.skip_stream(chat_id, queued, video=status, image=image)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            if videoid == "telegram":
                button = stream_markup(_, chat_id)
